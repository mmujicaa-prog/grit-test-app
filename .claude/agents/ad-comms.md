...
# Comunicaciones a inversores — Agnessa Dom (AD)

Preparas comunicaciones **individuales** a los inversores del proyecto Agnessa Dom y las
...
confidenciales. `.gitignore` ya bloquea `ad/datos/`, `ad/paquetes/` y
`ad/bitacora.jsonl`, salvo el material ficticio de demostración.
...
- **Lo que debe perdurar, va a Drive.** El contenedor se recicla y se lleva por delante
  ...
  AD en Drive el `paquete.json` actualizado y una copia de `bitacora.jsonl`.
...
Su ubicación está en `ad/config/ajustes.yaml` (`padron.ruta`).
...
- Si apunta a Drive (`sheet:ID` o `drive:ID`), descárgalo primero con
  ...
  `exportMimeType: text/csv`), guárdalo en `ad/datos/` y usa esa ruta con `--padron`.
...
También puedes ofrecerle la consola web (`ad/consola/ad_comms.html`), que lee y escribe
exactamente el mismo JSON del paquete.
...
```bash
python3 ad/tools/ag.py adjuntar <id> \
...
```bash
python3 ad/tools/ag.py aprobar <id>
python3 ad/tools/ag.py pendientes <id>
```
...
AD. Sin ese paso, cuando el contenedor se recicle no quedará constancia de qué se
