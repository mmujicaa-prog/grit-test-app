...
# Comunicaciones a inversores — Navalmanzano

Preparas comunicaciones **individuales** a los inversores del proyecto Navalmanzano y las
...
confidenciales. `.gitignore` ya bloquea `navalmanzano/datos/`, `navalmanzano/paquetes/` y
`navalmanzano/bitacora.jsonl`, salvo el material ficticio de demostración.
...
- **Lo que debe perdurar, va a Drive.** El contenedor se recicla y se lleva por delante
  los archivos locales. Al terminar una tanda de borradores, sube a la carpeta de
  Navalmanzano en Drive el `paquete.json` actualizado y una copia de `bitacora.jsonl`.
...
Su ubicación está en `navalmanzano/config/ajustes.yaml` (`padron.ruta`).
...
- Si apunta a Drive (`sheet:ID` o `drive:ID`), descárgalo primero con
  ...
  `exportMimeType: text/csv`), guárdalo en `navalmanzano/datos/` y usa esa ruta con `--padron`.
...
```bash
python3 navalmanzano/tools/ag.py generar \
  --id 2026-09-avance-obra \
  --titulo "Avance de obra — septiembre 2026" \
  --plantilla avance_proyecto.html \
  --asunto "Navalmanzano · Avance de obra {{periodo}} — {{nombre}}" \
...
También puedes ofrecerle la consola web (`navalmanzano/consola/navalmanzano_comms.html`), que
...
```bash
python3 navalmanzano/tools/ag.py adjuntar <id> \
...
```bash
python3 navalmanzano/tools/ag.py aprobar <id>
python3 navalmanzano/tools/ag.py pendientes <id>
```
...
Sube a Drive el `paquete.json` actualizado y la `bitacora.jsonl`, a la carpeta de
Navalmanzano. Sin ese paso, cuando el contenedor se recicle no quedará constancia de qué se
